...
function iniciaJogo() {
  ...
  jogo.numeroAleatorio =
    geraNumeroInteiroAleatorio(configuracao.minimoAleatorio,
                               configuracao.maximoAleatorio);
  jogo.tentativas = [];
  jogo.incio = Math.floor(Date.now / 1000);
  setInterval(mostraTempoJogo, 1000);
...
function terminaJogo(resultadoTentativaFinal) {
  ...
  else if (document.getElementById(BOTAO_CANCELA_JOGO).clicked = true){
    alert(RESULTADO_CANCELOU);
  }
  else {
